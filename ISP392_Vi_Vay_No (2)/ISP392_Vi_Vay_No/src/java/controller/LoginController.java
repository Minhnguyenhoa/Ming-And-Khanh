    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        if (accountFound != null) {
            ...
            if (isValidCaptcha) {
                ...
                session.setAttribute("waiting_USER", accountFound);
                boolean isAdmin = accountDAO.isAdmin(username);
                session.setAttribute("waiting_Admin", isAdmin);
                String token = new Token().generateRandomToken(18);
                session.setAttribute("waiting_token", token);

                String loginUrl = "http://" + request.getServerName() + ":" + request.getServerPort()
                        + request.getContextPath() + "/login?t=" + token;
                new Mail().sendEmail(accountFound.getEmailAddress(), "Login", "Click here to login: " + loginUrl);
